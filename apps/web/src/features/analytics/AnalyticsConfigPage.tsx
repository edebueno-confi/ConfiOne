...
import { listAnalyticsSourceConfig, listCsOpsImportRuns, runCsOpsMigration, triggerCsOpsSpreadsheetImport, upsertAnalyticsSourceConfig, type CsOpsImportRun } from './analytics-api';
...
export function AnalyticsConfigPage() {
  ...
  useEffect(() => { load(); void listCsOpsImportRuns().then(setCsOpsRuns).catch(() => setCsOpsRuns([])); }, []);
  const save = async () => {
