  ...
  return <AnalyticsHdDomainFrame title="Financeiro" description="Recebíveis, aging e posição financeira atual." source="OMIE · Contas a Receber" state={dataState} headerAside={financeSourceMeta}>
    ...
    {dataState?.status === 'empty' ? <MinimalState title="Nenhum dado financeiro" description="A consulta funcionou, mas não há registros neste recorte. Experimente ampliar o período." /> : <AnalyticsDomainTabs activeId={subTab} onChange={setSubTab} tabs={[
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        ...
        <ChartCard title="Carteira cruzada com o financeiro" description="Valores em aberto ligados aos clientes. O cruzamento confirma CNPJ único ou decisão humana com evidência; semelhança de nome serve apenas de pista.">
          <div className="mb-3 flex flex-wrap items-center gap-2">
            ...
            <Tag label={`Pendente de conciliação: ${formatCurrencyBRL(snapshot.csReconciliation.unmatchedBalance)}`} tone={snapshot.csReconciliation.unmatchedBalance > 0 ? 'warning' : 'neutral'} />
            <MetricInfo text="Saldo ainda sem vínculo confirmado. A busca também aponta candidatas por razão social e nome fantasia, sem conciliá-las automaticamente." />
          </div>
          ...
          <p className="mt-3 text-[11px] leading-4 text-[color:var(--minimal-text-tertiary)]"><strong>Sem status CS</strong>: empresa existe no HubSpot (CNPJ bateu), mas sem o campo de status de cliente preenchido. <strong>Pendente de conciliação</strong>: não há vínculo confirmado; pode existir uma candidata por razão social ou nome fantasia. <strong>Grupo de Empresas</strong> e demais: status vindo do HubSpot.</p>
          <div className="mt-3">
            <button type="button" onClick={() => void toggleUnmatched()} disabled={loadingUnmatched} className="rounded-lg border border-[color:var(--minimal-border-strong)] px-3 py-1.5 text-xs font-medium text-[color:var(--minimal-text)] hover:bg-[color:var(--minimal-surface-muted)] disabled:opacity-60">{loadingUnmatched ? 'Carregando...' : unmatched ? 'Ocultar pendências de conciliação' : 'Ver pendências OMIE ↔ HubSpot'}</button>
          </div>
          {unmatched ? (unmatched.length === 0 ? <p className="mt-3 text-xs text-[color:var(--minimal-text-tertiary)]">Nenhuma empresa sem cadastro neste recorte.</p> : <>
            <p className="mt-3 mb-2 text-[11px] leading-4 text-[color:var(--minimal-text-tertiary)]">A conciliação automática usa CNPJ normalizado. Para revisão, a busca também consulta nome, razão social e nome fantasia no cache do HubSpot; uma candidata não cria vínculo por si só.</p>
            <div className="overflow-x-auto"><table className="gso-analytics-responsive-table w-full min-w-[560px] text-sm"><thead><tr className="border-b border-[color:var(--minimal-border)] text-left text-[11px] font-semibold uppercase tracking-wide text-[color:var(--minimal-text-tertiary)]"><th className="py-2">Empresa (OMIE)</th><th className="py-2">CNPJ</th><th className="py-2 text-right">Títulos</th><th className="py-2 text-right">Saldo</th><th className="py-2 text-right">Vencido</th><th className="py-2">Motivo</th></tr></thead><tbody>{unmatched.map((row, index) => <tr key={`${row.taxId ?? row.client}-${index}`} className="border-b border-[color:var(--minimal-border)] last:border-0"><td data-label="Empresa" className="py-2 text-[color:var(--minimal-text)]">{row.client}{row.tradeName ? <span className="mt-0.5 block text-[11px] text-[color:var(--minimal-text-tertiary)]">Nome fantasia: {row.tradeName}</span> : null}</td><td data-label="CNPJ" className="py-2 tabular-nums text-[color:var(--minimal-text-tertiary)]">{row.taxId ?? '—'}</td><td data-label="Títulos" className="py-2 text-right tabular-nums text-[color:var(--minimal-text-secondary)]">{row.titles.toLocaleString('pt-BR')}</td><td data-label="Saldo" className="py-2 text-right tabular-nums font-medium text-[color:var(--minimal-text)]">{formatCurrencyBRL(row.balance)}</td><td data-label="Vencido" className="py-2 text-right tabular-nums text-[color:var(--minimal-danger-text)]">{formatCurrencyBRL(row.overdueBalance)}</td><td data-label="Motivo" className="py-2">{row.nameMatches > 0 ? <Tag label="Candidata encontrada no HubSpot" tone="warning" /> : <Tag label="Sem candidata encontrada" tone="neutral" />}</td></tr>)}</tbody></table></div>
          </> ) : null}
        </ChartCard>
      </div>
