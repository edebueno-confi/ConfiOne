...
export function AnalyticsReportExport({ open, period, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    setState('loading');
    setError('');
    Promise.all([getCeoSnapshot(filters), getCommercialSnapshot(filters), getCsSnapshot(filters), getFinanceSnapshot(filters)])
