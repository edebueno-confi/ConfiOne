-- SUPPORT-KPIS — cobertura de atividade no indicador de parados
--
-- O QA visual pegou, de novo, o mesmo defeito e no mesmo dia. Contra uma base
-- sem `last_activity_at`, "Parados dentro da fila" publicava **0** com a
-- tipografia de um zero medido. Zero ali afirma "nenhum atendimento parado",
-- quando a verdade é "não temos como saber".
--
-- Eu havia corrigido exatamente isso em `rpc_analytics_support_queue_health` e
-- não apliquei a mesma regra aqui. A lição que sobra é que a checagem de
-- cobertura precisa acompanhar a medida, e não a função: todo indicador que
-- depende de um campo opcional carrega o risco de publicar ausência como fato.
  ...
  select jsonb_build_object(
    ...
    'kpis', jsonb_build_object(
      ...
      'stagnant_in_queue', app_private.kpi_entry(
        bl.stagnant_in_queue::numeric, 'ticket_last_activity_at', sst.state, sst.reason),
