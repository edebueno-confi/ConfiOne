-- SUPPORT-KPIS — dois indicadores que publicavam número enganoso
--
-- 1. Passivo e parados publicavam número antes de existir a decisão
-- -----------------------------------------------------------------
-- Enquanto nenhum pipeline tem papel definido, tudo conta como fila. Nesse
-- estado `dormant_backlog` devolvia **0** e `stagnant_in_queue` devolvia o total
-- de parados da base inteira. Os dois rótulos mentem: "Passivo fora da fila: 0"
-- afirma que nada está fora, e "Parados dentro da fila" mostra parados que não
-- se sabe se estão dentro.
--
-- Ambos passam a `unavailable` até haver classificação. Indicador que só existe
-- depois de uma decisão não deve publicar número antes dela.
--
-- 2. Resolução instantânea contaminava a mediana
-- ----------------------------------------------
-- 81 dos 351 encerramentos do último mês — 23% — têm data de encerramento igual
-- à de abertura. É automação fechando na entrada, não atendimento, e puxa a
-- mediana para 0,1 dia.
--
-- Excluir seria inventar regra: pode haver atendimento legitimamente resolvido no
-- ato. O que a função passa a fazer é **publicar a contagem**, para que a tela
-- possa dizer quantos entraram no cálculo dessa forma. Quem lê decide o peso.
...
begin
  ...
  -- Passivo e parados-na-fila só significam alguma coisa depois que alguém
  -- decidiu quais pipelines são fila de trabalho.
  split_state as (
    select
      case when v_classificados = 0 then 'unavailable' else 'available' end as state,
      case when v_classificados = 0 then 'queue_role_unclassified' else null end as reason
  ),
  -- Cobertura do campo de ultima atividade dentro da fila de trabalho. Sem ele
  -- nao ha como separar quem esta parado de quem esta sendo tratado.
  activity_coverage as (
    select
      count(*) filter (where is_open and conta_como_fila)::integer as open_in_queue,
      count(*) filter (where is_open and conta_como_fila and last_activity_at is not null)::integer as with_activity
    from scoped
  ),
  stagnation_state as (
    select
      case when ac.open_in_queue = 0 then 'unavailable'
  ...
  select jsonb_build_object(
    ...
    'kpis', jsonb_build_object(
      ...
      'dormant_backlog', app_private.kpi_entry(
        case when v_classificados = 0 then null else bl.dormant_tickets::numeric end,
        'ticket_state_open_now', sps.state, sps.reason),
      'stagnant_in_queue', app_private.kpi_entry(
        case when v_classificados = 0 then null else bl.stagnant_in_queue::numeric end,
        'ticket_last_activity_at',
        case when v_classificados = 0 then 'unavailable' else sst.state end,
        case when v_classificados = 0 then 'queue_role_unclassified' else sst.reason end),
      -- Encerramentos com data igual à de abertura. Publicado para que a mediana
      -- possa ser lida sabendo quanto dela vem de fechamento automático.
      'instant_resolutions', app_private.kpi_entry(
        rs.instant_resolutions::numeric, 'ticket_resolved_at', rst.state, rst.reason),
