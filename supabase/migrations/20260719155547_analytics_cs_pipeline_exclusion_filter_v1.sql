-- Permite retirar temporariamente pipelines do recorte CS sem desativá-los na
-- configuração persistida. A seleção é somente leitura e vale para a consulta.

drop function if exists public.rpc_analytics_cs_snapshot(date, date, text, text, text[]);
drop function if exists public.rpc_analytics_cs_snapshot(date, date, text, text);
...
with cfg as (
  ...
  select pipeline_id, coalesce(nullif(source_type, ''), 'Sem fonte') source_label, count(*)::integer ticket_count
  from scoped group by pipeline_id, source_label
), pipelines as (
  select s.pipeline_label label, s.pipeline_id, count(*)::integer ticket_count,
    coalesce((select jsonb_agg(jsonb_build_object('label', ps.source_label, 'ticket_count', ps.ticket_count) order by ps.ticket_count desc, ps.source_label)
...
comment on function public.rpc_analytics_cs_snapshot(date, date, text, text, text[]) is
  'Snapshot de CS filtrável por período, status, prioridade e exclusão temporária de pipelines ativos.';

-- Compatibilidade para clientes antigos que ainda chamam o contrato V1 sem
