-- Cockpit financeiro V1: a API OMIE passa a ser a fonte de verdade.
-- rpc_analytics_finance_snapshot escolhe a fonte ativa (API se houver titulos da
-- API; caso contrario a planilha mais recente) para evitar dupla contagem entre
-- planilha e API, e agrega metricas de gestao: posicao atual da carteira, aging
-- por faixa de dias, previsibilidade de recebiveis futuros, categorias e maiores
-- devedores. Todo o calculo permanece no backend.
...
comment on function public.rpc_analytics_finance_snapshot(date, date, text, text, text) is
  'Cockpit financeiro: fonte ativa (API OMIE preferida; planilha como fallback), posicao da carteira, aging por faixa de dias, previsibilidade de recebiveis, categorias e maiores devedores. Calculo no backend.';
