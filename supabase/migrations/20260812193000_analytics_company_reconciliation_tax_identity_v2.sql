-- DASHBOARD-RECONCILIATION-TAX-IDENTITY-V2
-- Usa as mesmas chaves fiscais alternativas do cadastro HubSpot no fluxo de
-- governanca. A busca continua sendo sugestao; nenhuma associacao e gravada.
  ...
  with client_index as (
    ...
    left join lateral (
      with candidate_companies as (
        ...
        from (
          select c.company_id, c.name,
            nullif(regexp_replace(coalesce(nullif(c.raw ->> 'cnpj__chave_unica_', ''), nullif(c.raw ->> 'cnpj_id', ''), c.tax_id, ''), '[^0-9]', '', 'g'), '') as tax_id,
            0::integer as candidate_source
          from public.hubspot_companies c
          where c.company_id = r.confirmed_company_id
          union all
          select c.company_id, c.name,
            nullif(regexp_replace(coalesce(nullif(c.raw ->> 'cnpj__chave_unica_', ''), nullif(c.raw ->> 'cnpj_id', ''), c.tax_id, ''), '[^0-9]', '', 'g'), ''),
            1::integer
          from public.hubspot_companies c
          where r.source_tax_id is not null
            and nullif(regexp_replace(coalesce(nullif(c.raw ->> 'cnpj__chave_unica_', ''), nullif(c.raw ->> 'cnpj_id', ''), c.tax_id, ''), '[^0-9]', '', 'g'), '') = r.source_tax_id
          union all
          select c.company_id, c.name,
            nullif(regexp_replace(coalesce(nullif(c.raw ->> 'cnpj__chave_unica_', ''), nullif(c.raw ->> 'cnpj_id', ''), c.tax_id, ''), '[^0-9]', '', 'g'), ''),
            2::integer
        ...
        select
          ...
          c.tax_id,
          case
            when r.source_tax_id is not null and c.tax_id = r.source_tax_id then 1.0
      ...
      select jsonb_agg(jsonb_build_object(
        ...
        'reason', case
          when r.source_tax_id is not null and s.tax_id = r.source_tax_id then 'cnpj_exato'
        ...
        'matched_fields', to_jsonb(array_remove(array[
          case when r.source_tax_id is not null and s.tax_id = r.source_tax_id then 'CNPJ' end,
        ...
        'differences', jsonb_strip_nulls(jsonb_build_object(
          ...
          'tax_id', case when r.source_tax_id is not null and s.tax_id <> r.source_tax_id then jsonb_build_object('omie', r.source_tax_id, 'hubspot', s.tax_id) end
        )),
...
comment on function public.rpc_analytics_company_reconciliation_queue(integer, integer) is
  'Fila de governanca HubSpot OMIE com chaves fiscais alternativas publicadas pelo HubSpot; sugestoes permanecem pendentes ate decisao humana.';
