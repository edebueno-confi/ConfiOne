-- Reaplica o snapshot CS no banco que ja possui a migration anterior.
-- O detalhamento de responsaveis e consolidado por pipeline, nunca por owner_id.
...
with cfg as (
  ...
  select t.*, cfg.pipeline_label,
    coalesce(s.is_closed, false) is_closed,
    s.label stage_label,
    s.display_order,
    s.is_closed stage_is_closed
  ...
  select count(*)::integer total_tickets,
    count(*) filter (where not is_closed)::integer open_tickets,
  ...
  select coalesce(stage_label, 'Status sem rotulo') label,
    pipeline_id,
    pipeline_label,
    coalesce(pipeline_stage, '') stage_id,
    coalesce(display_order, 0) display_order,
    coalesce(stage_is_closed, false) is_closed,
    count(*)::integer ticket_count
  from scoped
  group by coalesce(stage_label, 'Status sem rotulo'), pipeline_id, pipeline_label,
    coalesce(pipeline_stage, ''), coalesce(display_order, 0), coalesce(stage_is_closed, false)
), statuses as (
  select label,
    coalesce(string_agg(distinct nullif(stage_id, ''), ',' order by nullif(stage_id, '')), '') stage_id,
    min(display_order)::integer display_order,
    bool_and(is_closed) is_closed,
    sum(ticket_count)::integer ticket_count,
    coalesce(jsonb_agg(jsonb_build_object(
      'pipeline_id', pipeline_id,
      'pipeline_label', pipeline_label,
      'stage_id', stage_id,
      'ticket_count', ticket_count
    ) order by ticket_count desc, pipeline_label), '[]'::jsonb) pipeline_breakdown
  from status_by_pipeline
  group by label
), monthly as (
  select coalesce(created.month_start, closed.month_start) month_start,
    coalesce(created.created_count, 0) created_count,
  ...
  from (
    ...
    from scoped where hs_created_at is not null group by 1
  ) created
  full outer join (
    select date_trunc('month', hs_closed_at)::date month_start, count(*)::integer closed_count
    from scoped where hs_closed_at is not null group by 1
  ) closed using (month_start)
), sources as (
  select coalesce(nullif(source_type, ''), 'Sem fonte') label,
    count(*)::integer ticket_count
  from scoped group by 1 order by ticket_count desc
), pipeline_sources as (
  select pipeline_id,
    coalesce(nullif(source_type, ''), 'Sem fonte') source_label,
    count(*)::integer ticket_count
  from scoped group by pipeline_id, source_label
), pipelines as (
  select s.pipeline_label label,
    s.pipeline_id,
    count(*)::integer ticket_count,
    coalesce((select jsonb_agg(jsonb_build_object(
      'label', ps.source_label, 'ticket_count', ps.ticket_count
    ) order by ps.ticket_count desc, ps.source_label)
    from pipeline_sources ps where ps.pipeline_id = s.pipeline_id), '[]'::jsonb) source_summary
  ...
  select coalesce(nullif(o.full_name, ''), nullif(o.email, ''), 'Sem responsavel') owner_name,
    case when count(distinct nullif(s.owner_id, '')) = 1
      then min(nullif(s.owner_id, '')) else null end owner_id,
    s.pipeline_id,
    s.pipeline_label,
    count(*)::integer ticket_count
  from scoped s
  left join public.hubspot_owners o on o.owner_id = s.owner_id
  group by owner_name, s.pipeline_id, s.pipeline_label
), owners as (
  select owner_name,
    case when count(distinct nullif(owner_id, '')) = 1
      then min(nullif(owner_id, '')) else null end owner_id,
    sum(ticket_count)::integer ticket_count,
    coalesce(jsonb_agg(jsonb_build_object(
      'pipeline_id', pipeline_id,
      'pipeline_label', pipeline_label,
