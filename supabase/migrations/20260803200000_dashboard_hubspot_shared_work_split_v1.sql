-- DASHBOARD-HUBSPOT-SHARED-WORK-SPLIT-V1
-- Divide o item compartilhado em unidades retomáveis. Empresas e owners são
-- paginados; catálogo permanece separado para não manter uma invocação longa.
-- Forward-only: não remove histórico nem altera snapshots publicados.
...
begin
  ...
  if exists (
    ...
    where provider = 'hubspot' and domain_key = 'all'
  ...
  if p_mode is null and exists (
    select 1 from public.hubspot_sync_runs
    where provider = 'hubspot' and status in ('success', 'succeeded') and watermark_advanced
  ...
  insert into public.hubspot_sync_runs(
    provider, domain_key, domains, mode, status, triggered_by, requested_by,
    correlation_id, heartbeat_at, source_updated_after_ms, source_pagination_complete
  ) values (
    'hubspot', 'all', v_domains, v_mode, 'queued', v_actor, v_actor,
    coalesce(p_correlation_id, extensions.gen_random_uuid()), timezone('utc', now()), v_after, false
  ...
  select v_run.id, c.domain_key, c.object_type, c.hubspot_pipeline_id,
         coalesce(c.hubspot_pipeline_label, c.label), 0, 4102444800000
  from public.analytics_source_config c
  where c.is_active and not c.is_archived and c.domain_key = any(v_domains)
  on conflict do nothing;

  -- Cada unidade tem uma invocação curta, cursor próprio e telemetria
  -- persistida antes do checkpoint. O catálogo não fica misturado à carga de
  -- empresas/owners e não repete a leitura completa de nenhuma dessas bases.
  insert into public.analytics_cs_sync_work_items(
    parent_run_id, domain_key, object_type, pipeline_id, pipeline_label,
    range_start_ms, range_end_ms
  ) values
    (v_run.id, 'shared', 'shared_companies', 'shared_companies', 'Empresas HubSpot', 0, 4102444800000),
    (v_run.id, 'shared', 'shared_owners', 'shared_owners', 'Owners HubSpot', 0, 4102444800000),
    (v_run.id, 'shared', 'shared_catalog', 'shared_catalog', 'Catálogo de pipelines HubSpot', 0, 4102444800000);

  select count(*)::integer into v_count
  from public.analytics_cs_sync_work_items where parent_run_id = v_run.id;
  if v_count = 0 then
  ...
  update public.hubspot_sync_runs
  set pipelines_total = v_count, source_state = null
  where id = v_run.id;

  return jsonb_build_object(
    'status', 'queued', 'run_id', v_run.id, 'correlation_id', v_run.correlation_id,
    'mode', v_mode, 'domains', v_domains, 'work_items_total', v_count,
...
comment on function public.rpc_analytics_hubspot_start_run(text, text, uuid) is
  'Inicia carga HubSpot incremental/full com pipelines e unidades compartilhadas separadas, retomáveis e observáveis.';
