...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const telemetry = createSyncRequestTelemetryBuffer({ provider: 'hubspot', hubspotRunId: item.run_id, workItemId: item.work_item_id, correlationId: item.correlation_id, pageNumber: Number(item.page_number) });
    ...
    try {
      ...
      if (item.object_type === 'ticket') {
        ...
        const owners = await fetchOwners(token, telemetry.observer); if (owners.length) { const rows=owners.map((o)=>({parent_run_id:item.run_id,owner_id:o.ownerId,email:o.email,first_name:o.firstName,last_name:o.lastName,full_name:o.fullName,archived:o.archived,raw:o.raw,synced_at:new Date().toISOString()})); const {error}=await client.from('analytics_hubspot_owner_staging').upsert(rows,{onConflict:'parent_run_id,owner_id'}); if(error) throw error; }
