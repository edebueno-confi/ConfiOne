...
Deno.serve(async (req) => {
  ...
  try {
    const { data: claimed, error: claimError } = await client.rpc('rpc_analytics_hubspot_claim_work_item', { p_worker_id: workerId, p_lease_seconds: 90 });
