...
test('monta requisição paginada da API Omie sem expor segredo', () => {
  ...
  assert.deepEqual(request.param, [{ nPagina: 2, nRegPorPagina: 500 }]);
...
test('extrai página e metadados de contas a receber', () => {
  const page = extractOmieReceivablesPage({ pagina: 1, total_de_paginas: 3, lista_contas_receber: [{ nCodTitulo: 10 }] });
  assert.deepEqual(page.rows, [{ nCodTitulo: 10 }]);
...
test('faz retry apenas para falhas transitórias da API Omie', async () => {
  ...
  const rows = await fetchOmieReceivables({ appKey: 'a', appSecret: 'b' }, async () => {
    ...
    return new Response(JSON.stringify({ pagina: 1, total_de_paginas: 1, lista_contas_receber: [{ nCodTitulo: 10 }] }), { status: 200 });
  }, { timeoutMs: 1000, maxRetries: 1 });
  assert.equal(calls, 2);
  assert.deepEqual(rows, [{ nCodTitulo: 10 }]);
});
