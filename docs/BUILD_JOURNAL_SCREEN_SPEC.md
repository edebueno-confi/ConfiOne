...
Atualização de 2026-05-17: a aba `Documentos oficiais` foi implementada como camada narrativa dentro do Diário de Construção. A tela organiza categorias documentais, explica o papel de cada grupo na construção do produto e aponta apenas para documentos já whitelisted em `/admin/product-docs?doc=...`; documentos ainda não expostos na whitelist aparecem como pendentes, sem link e sem simular leitura. Esta implementação não criou backend, migration, RPC, view, RLS, parser dinâmico, file explorer, busca backend ou contrato novo.

Atualização de 2026-05-17: a aba `Documentos oficiais` passou a abrir documentos whitelisted dentro do próprio contexto narrativo do Diário, usando o mesmo reader compartilhado de `/admin/product-docs`. `/admin/product-docs` continua como leitor oficial controlado, com busca local, categorias, whitelist e deep link `?doc=` preservados. A mudança não criou segunda whitelist, não criou parser concorrente, não lê filesystem e não alterou backend, migration, RPC, view, RLS, tabela ou Supabase.
