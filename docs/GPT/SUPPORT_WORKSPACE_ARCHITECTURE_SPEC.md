...
- A UI continua restrita a contratos reais ja materializados para fila, detalhe, timeline, contexto do cliente, classificacao, anexos, knowledge links, handoff tecnico e SLA; o lote permaneceu estritamente frontend, sem migration, sem schema novo, sem RPC nova, sem alteracao de RLS/policy e sem leitura direta de tabela base.
...
## Internal Actions Backend Foundation V1
- o backend do workspace agora possui um dominio formal de `internal_actions` separado do ticket principal e separado do dominio tecnico de engenharia.
- esse dominio nasce apenas no backend nesta fase:
  - sem drawer novo
  - sem rota nova
  - sem layout novo
  - sem acao fake no frontend
...
- consequencia arquitetural:
  ...
  - a UI futura precisa explicitar essa separacao sem misturar lifecycle de ticket com lifecycle do subfluxo interno
