...
- O lote `Internal Actions Backend Foundation V1` criou o domínio neutro `internal_actions` para subfluxos internos de ticket, com catálogo governado de áreas, memberships por área, ledger append-only, views/RPCs dedicadas, `ticket_events` internos e pgTAP, sem UI nova, sem bridge com Engenharia e sem alterar `ticket.status` no V1.
