...
- `publishable_now`: `0`.
- `publishable_with_minor_cleanup`: `42`.
- `blocked_critical`: `12`.
- `duplicate_or_obsolete`: `0`.
- Artigos publicados nesta execucao: `42`.
...
- `como-alterar-ou-aprovar-os-produtos-de-uma-solicitacao` - Como alterar ou aprovar os produtos de uma solicitação?: `blocked_critical` (manual_review_required).
...
## Evidencia da Central Publica e imagens

- Artigos Octadesk publicos no read model: `42`.
- Referencias de assets publicos: `97`.
- Objetos fisicos encontrados no bucket publico: `97/97`.
- Referencias sem objeto correspondente: `0`.
- Assets publicos sem texto alternativo: `0`.
- Artigos publicos sem categoria: `0`.
- A leitura runtime do artigo `configurando-parametrizacao-geral` confirmou as `9` imagens apos carregamento progressivo.
- O conteudo continua sendo servido pelos read models publicos; a interface nao expoe os doze artigos bloqueados.
