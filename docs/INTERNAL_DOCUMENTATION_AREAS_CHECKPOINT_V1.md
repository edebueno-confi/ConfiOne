...
Atualização de 2026-05-17: a aba `Documentos oficiais` deixou de ser placeholder e passou a funcionar como camada narrativa do Diário de Construção. Ela não substitui `/admin/product-docs`: organiza categorias, explica por que cada grupo documental existe e aponta apenas para documentos já whitelisted no leitor oficial. Documentos ainda não expostos na whitelist aparecem como pendentes, sem ação fake.

Atualização de 2026-05-17: a aba `Documentos oficiais` passou a embutir o mesmo reader controlado usado por `/admin/product-docs`, preservando a experiência narrativa do Diário sem criar leitor concorrente. `/admin/product-docs` permanece como fonte oficial completa, com busca local, categorias, whitelist e deep links `?doc=`. O Diário usa metadados narrativos próprios, mas o corpo dos documentos continua vindo da fonte estática whitelisted do Product Docs.
